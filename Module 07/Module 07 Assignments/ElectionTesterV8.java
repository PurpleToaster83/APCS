/**
 * Purpose:  This class defines a Election tester that uses array lists
 *
 ...
 */
 ...
 public class ElectionTesterV8 {
    public static void deleteCand(ArrayList<Candidate> candidates, int pos) {
        candidates.remove(pos);
    }

    public static void deleteCand(ArrayList<Candidate> candidates, String name) {
        int location = 0;
    ...
    public static void addCand(ArrayList<Candidate> candidates, int pos, String name, int votes) {
        candidates.add(pos, new Candidate(name, votes));
    }

    public static void insertBefore(ArrayList<Candidate> candidates, String nameFind, String name, int votes) {
        int size = candidates.size();
        for(int loc = 0; loc < size; loc++) {
            if(candidates.get(loc).getName().equals(nameFind)) {
                candidates.add(loc, new Candidate(name, votes));
                break;
            } 
        }
    ...
    public static void printTable(ArrayList<Candidate> candidates, int total) {
        ...
        for(Candidate active : candidates) {
            System.out.printf("%s%13d\t\t%3.2f\n", active.getName(), active.getVotes(), (100 * active.getVotes() / (double) total));
        }
    }
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        System.out.println();
        addCand(candidates, 2, "Candidate R", 100);
        printTable(candidates, totalVotes(candidates));
        System.out.println();
        System.out.println("Add Candidate Prior:");
        System.out.println();
        insertBefore(candidates, "Candidate B", "Candidate W", 345);
        printTable(candidates, totalVotes(candidates));
        System.out.println();
        System.out.println("Delete Candidate 4:");
        System.out.println();
        deleteCand(candidates, 3);
        printTable(candidates, totalVotes(candidates));
        System.out.println();
        System.out.println("Delete Candidate A");
        System.out.println();
        deleteCand(candidates, "Candidate A");
        printTable(candidates, totalVotes(candidates));
    }
}
